...
struct PhysicalParameters {
    ...
    double outflow_pressure = 1.0;
    double Mach_inf = 0.0; // для характеристических BC
    ...
    double T_init = 300.0;     // Начальная температура ghost-ячеек (K)
...
struct Face {
    int left, right;          // индексы ячеек (right = -1 для границы)
    int n0, n1;               // индексы узлов грани
    double nx, ny;            // единичная нормаль, направленная от left к right
    double length;            // длина грани
    double mx, my;            // координаты центра грани
    BoundaryType bc;          // тип граничного условия (добавьте в enum)
    bool is_boundary() const { return right < 0; }
};

struct Cell {
    std::vector<int> node_ids;   // индексы узлов (для треугольника – 3)
    std::vector<int> face_ids;   // индексы граней, принадлежащих ячейке
    double vol;                  // площадь ячейки
    double cx, cy;               // центроид
    Conserved U;                 // консервативные переменные
    Conserved res;               // невязка
};
...
struct Mesh {
    ...
    void zero_res() {
        for (auto& c : cells) c.res = { 0,0,0,0 };
    }
};
