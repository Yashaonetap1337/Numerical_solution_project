...
Config readConfig(const std::string& filename) {
    ...
    // Убираем UTF-8 BOM (EF BB BF) если он есть в начале файла.
    // Visual Studio сохраняет файлы с BOM по умолчанию.
    {
        char bom[3] = { 0 };
        file.read(bom, 3);
        if (!((unsigned char)bom[0] == 0xEF &&
            ...
            file.seekg(0);  // BOM не найден — возвращаемся в начало
        }
    }
    ...
    while (std::getline(file, line)) {
        ...
        try {
            if (section == "PhysicalParameters") {
                ...
                else if (key == "T_init") cfg.phys.T_init = v;
    ...
    // Mader allows P=0 on the unexploded side (EOS floors it to P_min internally)
