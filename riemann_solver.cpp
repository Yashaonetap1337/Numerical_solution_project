...
State solve_general_riemann_problem(const State& W_L, const State& W_R, double xi, const Config& cfg, const ApproximationType approx_type) {
    ...
    for (int i = 0; i < MAX_ITERATIONS; ++i) {
        ...
        // правильная проверка сходимости
    ...
    // ИСПРАВЛЕНИЕ: поперечная скорость — upwind по знаку u_star
    // Решатель Римана работает только с нормальной компонентой (u).
    // Поперечная компонента (v) переносится контактным разрывом без изменений.
    solution_state.v = (xi <= u_star) ? W_L.v : W_R.v;
