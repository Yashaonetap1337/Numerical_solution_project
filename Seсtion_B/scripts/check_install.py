...
import numpy as np
from shock_reflection import detachment_wedge_angle, von_neumann_wedge_angle, beta_1_at_detachment, setup_wedge_simulation, detect_reflection

def main():
    print('=' * 60)
    print('CHECK 1: Теоретические значения')
    print('=' * 60)
    checks = [('M=5: theta_w^D ~ 27.77 deg', detachment_wedge_angle(5.0), 27.77, 0.05), ('M=5: theta_w^N ~ 20.86 deg', von_neumann_wedge_angle(5.0), 20.86, 0.05), ('M=4.96: beta_1^D = 39.33 deg (Chpoun)', beta_1_at_detachment(4.96), 39.33, 0.05), ('M=2: theta_w^N = None (M<2.202)', von_neumann_wedge_angle(2.0), None, None)]
    all_ok = True
    ...
    print('=' * 60)
    sim = setup_wedge_simulation(M=3.0, theta_w_deg=25.0, nx=120, ny=60, Lx=3.0, Ly=1.0, x_corner=0.5, cfl=0.4)
