...
def update_wedge_angle(sim: EulerSim, new_theta_w_deg: float, max_reinit_iters: int=50, verbose: bool=False):
    ...
    if verbose:
        print(f'  [update_wedge_angle] released={n_initial_pending}, reinit за {iters_used} итераций, orphan={n_orphan}')
    if n_orphan > 0:
        j_o, i_o = np.where(pending)
        fluid_cells = ~new_mask
        if fluid_cells.any():
            fallback = np.array([np.median(U[k][fluid_cells]) for k in range(4)])
            for j, i in zip(j_o, i_o):
                U[:, j, i] = fallback
        if n_orphan > 5:
            print(f'  [update_wedge_angle] WARNING: {n_orphan} orphan cells после {iters_used} итераций — использован median fallback.')
