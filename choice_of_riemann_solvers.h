...
inline Flux compute_interface_flux(const State& W_L, const State& W_R, const Config& cfg, char dir) {
    ...
    case RiemannSolverType::EXACT: {
        // ИСПРАВЛЕНИЕ: поворот скоростей для y-направления.
        // Решатель Римана всегда работает с u как нормальной скоростью.
        // Для y-интерфейса нормаль — это v, поэтому меняем u <-> v перед вызовом,
        // а после — возвращаем обратно.
        State WL = W_L, WR = W_R;
        if (dir == 'y') {
            std::swap(WL.u, WL.v);
            std::swap(WR.u, WR.v);
    ...
    case RiemannSolverType::ACOUSTIC: {
        // ИСПРАВЛЕНИЕ: аналогичный поворот для акустического солвера
        State WL = W_L, WR = W_R;
        if (dir == 'y') {
            std::swap(WL.u, WL.v);
            std::swap(WR.u, WR.v);
